...
struct node
{
	...
	int z;                      /**< z-coordinate of this cell */
	...
	for(x = 0; x < size; x++)
	{
		for(y = 0; y < size; y++)
		{
			...
			while(aux != NULL)
			{
				...
				{
					z = aux->z;
					add_node(&(cube[mod((x+1), size)][y]), NEIGHBOR, DEAD, z);
					add_node(&(cube[mod((x-1), size)][y]), NEIGHBOR, DEAD, z);
					add_node(&(cube[x][mod((y+1), size)]), NEIGHBOR, DEAD, z);
					add_node(&(cube[x][mod((y-1), size)]), NEIGHBOR, DEAD, z);
					add_node(&(cube[x][y]), NEIGHBOR, DEAD, mod((z+1), size));
					add_node(&(cube[x][y]), NEIGHBOR, DEAD, mod((z-1), size));
				}
				aux = aux->next;
			}
